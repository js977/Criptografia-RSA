...
/*
 * Implementação do Método do Crivo Quadrático
 */
class CrivoQuadratico {
private:
  // Em falta
  // Função para verificar se 'a' é um resíduo quadrático módulo 'p' usando o critério de Euler
  bool criterioEuler(const mpz_class&, const mpz_class&);

  // Em falta
  // Função para somar os elementos de uma linha de uma matriz
  mpz_class somaLinha(const vector<mpz_class>&);

  // Em falta
  //Função para ordena as linhas de uma matriz
  void ordenacao(vector<vector<mpz_class>>&);
  ...
  void gerar_a(mpz_class &, const mpz_class &);
  
  // Em falta
  // Função para calcular o ln(x) usando MPFR
  double ln_mpfr(const mpfr_t);
