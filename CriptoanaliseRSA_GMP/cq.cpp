...
mpz_class valorB(const mpfr_t n){
...
vector<mpz_class> baseFatorial(const mpz_class &B,const mpz_class &n) {
...
// Função de crivo
void sievingParcial(const mpz_class &n, const vector<mpz_class> &baseF,vector<vector<int>> &expVetor,vector<vector<mpz_class>> &id,vector<vector<int>> &expVetorX,vector<vector<mpz_class>> &idX,int resiQ) {
...
// Função de crivo
void sieving(const mpz_class &B, const mpz_class &n, const vector<mpz_class> &baseF,vector<vector<int>> &expVetor,vector<vector<mpz_class>> &id,int resiQ) {
...
void matrizMod2(vector<vector<mpz_class>> &id,vector<vector<int>> &expVetor,vector<vector<mpz_class>> &exp){
	//Declaração das variaveis
	size_t i,j;
...
void printMatrix(const vector<vector<mpz_class>>& matriz) {
...
vector<vector<mpz_class>> copiarParaMatriz(const vector<vector<mpz_class>>& matriz) {
	//Declaração das variaveis
	size_t i,j;
...
vector<vector<mpz_class>> transporMatriz(const vector<vector<mpz_class>>& matriz) {
...
vector<mpz_class> multiplicarMatrizVetor(const vector<vector<mpz_class>>& matriz, const vector<mpz_class>& vetor) {
...
mpz_class somaLinha(const vector<mpz_class>& linha) {
    mpz_class soma = 0;
    for (const auto& elemento : linha) {
        soma += elemento;
    }
...
void ordenacao(vector<vector<mpz_class>>& A){
...
void gaussEliminationMod2(vector<vector<mpz_class>>& A) {
    int numLi = A.size(); // número de linhas
    int numCols = A[0].size(); // número de colunas
	int j;
...
int backSolucao(vector<vector<mpz_class>>& A, vector<mpz_class>& x,int nSol) {
...
mpz_class yValor(vector<mpz_class> &baseF,const mpz_class &n, vector<vector<int>> &expVetor,const vector<vector<mpz_class>>& id,vector<mpz_class>& x){
...
mpz_class solucaoPrimo(vector<vector<mpz_class>>& exp,vector<mpz_class> &baseF,const mpz_class &n, vector<vector<int>> &expVetor,const vector<vector<mpz_class>>& id){
...
void crivoQuadratico(const mpz_class &n,mpz_class &p, mpz_class &q){
